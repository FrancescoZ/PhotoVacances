 ...
 */
//Le functionnement de cette classe est le meme que ListNameAdpter et List, il change seulement l'affichage dans la methode getiView
//donc on expliquera que une fois dans ListNameAdapter
public class ListPeriodAdapter extends ArrayAdapter<Album> implements Filterable {
    ...
    public ListPeriodAdapter(Context context, int resource, List<Album> albums) {
    ...
    public View getView(int position, View convertView, ViewGroup parent) {
        ...
        if (convertView == null) {
            ...
            TextView name = (TextView) v.findViewById(R.id.periodTxt);
            TextView periodtx = (TextView) v.findViewById(R.id.albumTxt);
        ...
        holder.albumTxt.setText(abl.getName().toString());
        ...
        protected FilterResults performFiltering(CharSequence constraint) {
            ...
            if (constraint == null || constraint.length() == 0) {
                ...
                for (Album p : albumList) {
                    if ((new SimpleDateFormat("dd/MM/yyyy").format(p.getPeriod().getFirst()) + " - "
                            + new SimpleDateFormat("dd/MM/yyyy").format(p.getPeriod().getSecond())).contains(constraint.toString()))
                        nAlbumList.add(p);
